...
class mj(Resource):
	...
	def index():
		def add_one(name,age,weight,bp,ailments,result):
			db.todos.insert_one({'Name': name, 'Age': age,'Weight':weight,'Blood Pressure':bp,'Ailments Noted':ailments,'Analysis':result})
#			return flask.jsonify(message="success")
			return "success"
		if request.method == 'POST':
			...
			mongo = add_one(name,age,weight,BloodPressure,". ".join(message),"".join(result))
			return str('Name: '+name+'<br/>'+'Age: '+age+'<br/>'+'Weight: '+weight+'<br/>'+'Blood Pressure: '+BloodPressure+'<br/>'+'Ailments Noted: '+". ".join(message)+'<br/>'+'Analysis: '+"".join(result)+'<br/>'+mongo)
